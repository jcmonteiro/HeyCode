...
describe("Transcript", () => {
  ...
  it("is immutable", () => {
    ...
    expect(() => { t.text = "changed" }).toThrow()
    expect(() => { t.meta.provider = "changed" }).toThrow()
  })
  ...
  it("rejects non-string text", () => {
    expect(() => new Transcript({ text: 42 })).toThrow(TypeError)
    expect(() => new Transcript({ text: null })).toThrow(TypeError)
  })
})
...
describe("Recording", () => {
  ...
  it("rejects empty outputPath", () => {
    ...
    expect(() => new Recording({ pid: 1, outputPath: 42 })).toThrow(TypeError)
  })
})
